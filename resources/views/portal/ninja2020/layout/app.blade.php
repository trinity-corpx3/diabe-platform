...
<head>
    ...
    @if ($company && $company->matomo_url && $company->matomo_id)
        ...
        <noscript>
            <p><img src="{{ auth()->guard('contact')->user()->company->matomo_url }}/matomo.php?idsite={{ auth()->guard('contact')->user()->company->matomo_id }}&amp;rec=1"
    ...
    @if(isset($company->account) && !$company->account->isPaid())
        <title>@yield('meta_title', '') — DIABE Platform</title>
